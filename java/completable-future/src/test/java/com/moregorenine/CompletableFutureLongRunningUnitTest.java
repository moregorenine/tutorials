...
public class CompletableFutureLongRunningUnitTest {
    ...
    @Test
    public void whenRunningCompletableFutureAsynchronously_thenGetMethodWaitsForResult() throws ExecutionException, InterruptedException {
        for (int i = 0; i < 10; i++) {
            Future<String> completableFuture = calculateAsync(i);
        }
...
//        assertEquals("Hello" + i, result);
    ...
    private Future<String> calculateAsync(int index) {
        CompletableFuture<String> completableFuture = new CompletableFuture<>();

        Executors.newCachedThreadPool()
                .submit(() -> {
                    Thread.sleep(5000);
                    completableFuture.complete("Hello" + index);
                    return null;
                });
